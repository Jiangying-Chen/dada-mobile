		...
		<view class="tip_box" :class="{ anScale: isAm }" v-if="showTip"
			...
			<view class="container" :style="{'margin-right': cotainerMargin + 'px',backgroundColor: bgColor,borderRadius: borderR + 'px',}">
				...
				<view class="close" @tap="tipHidden">
					<u-icon name="close" color="#aaaaff" size="18" v-if="closeColor"></u-icon>
					<u-icon name="close" size="18" v-else></u-icon>
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		methods: {
			tipHidden: function() {
				uni.setStorageSync("my_tips_2023", "true");
				this.showTip = false;
			},
			timeOut() {
				this.tipHidden();
			...
			init() {
				if (uni.getStorageSync("my_tips_2023")) return;
